...
class zeta(Scene):
    def construct(self):
        ...
        function_graph = ParametricFunction(
            parametric_function, t_range=[0, 100]
        ).set_stroke([YELLOW, BLUE])
        b = (
            MathTex(r"\zeta(s)=\sum_{n=1}^{\infty}\frac{1}{n^s}")
            .move_to(UP * 8)
            .scale(1.7)
        )
        ...
        self.play(
            FadeOut(
                VGroup(b, function_graph, complex_plane, stroke_color=BLUE), run_time=3
            )
        )
